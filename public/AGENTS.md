...
- While waiting on proof or extract, surface processing percent **when `get_mail_job` returns one** (`progress.millPercent` when present). Do not paste proof until the job is `ready` and `proofUrl` is signed. If millPercent is present, also wait until it is 100. If omitted, do not wait for millPercent. Details: https://www.usmail.ai/skills.md
